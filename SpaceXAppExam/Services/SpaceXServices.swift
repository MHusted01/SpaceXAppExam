...
// Service layer for all API requests
// Handles network calls and JSON decoding
struct SpaceXServices {

    // Endpoint URLs for the API
    private enum Endpoints {
        static let launchesV5 = URL(
            string: "https://api.spacexdata.com/v5/launches"
        )!
    ...
    // Shared decoder configured for API responses
    ...
    ///     url: URL
    /// - Returns: data
    /// - Throws: APIError as badResponse or badStatusCode
    ...
    private func fetch<T: Decodable>(_ type: T.Type, from url: URL) async throws
        -> T
    {
